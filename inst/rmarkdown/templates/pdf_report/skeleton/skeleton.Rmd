...
pretty_pvalue_compareGroups <- function(compareGroups_table_obj, digits = 3, sig_alpha = 0.05, background = 'yellow'){
  ...
  for (i in vars_to_adjust) {
    ...
    compareGroups_table_obj$descr[,i] <- pretty_pvalues(numeric_values, digits = digits, sig_alpha = sig_alpha, background = background)
  }
...
PT0N0_adj_fit_table <- run_pretty_model_output(
  ...
  event_in = NULL, event_level = 'Complete Response', latex_output = TRUE)
...
**Expalation of what plots/figures were created**
...
compareGroups::export2latex(pretty_pvalue_compareGroups(compareGroups::createTable(cycles_compare, digits.p = 4, show.p.mul = T), digits = 4), size = 'footnotesize', label = 'tab:compare_cycles', caption = 'Comparing Variables to Number of Cycles', header.labels = c('p.overall' = 'Overall P'), landscape = F)
...
compareGroups::export2latex(pretty_pvalue_compareGroups(compareGroups::createTable(PT0N0_compare, digits.p = 4, show.p.mul = T), digits = 4), size = 'small', label = 'tab:compare_PT0N0', caption = 'Comparing Variables to Downstaged to pT0N0', header.labels = c('p.overall' = 'Overall P'), landscape = F)
```
...
kable(PT0N0_adj_fit_table %>% select(-n), 'latex', escape = F, longtable = F, booktabs = TRUE, linesep = '', caption = 'Multivariable Logistic Regression Model Results for Downstaged to pT0N0') %>% 
  collapse_rows(c(1:2), row_group_label_position = 'stack', headers_to_remove = 1:2, latex_hline = 'major') %>%
...
kable(OS_adj_fit_table %>% select(-`n (events)`), 'latex', escape = F, longtable = F, booktabs = TRUE, linesep = '', caption = 'Multivariable Cox Proportional-Hazards Model for Overall Survival') %>% 
  collapse_rows(c(1:2), row_group_label_position = 'stack', headers_to_remove = 1:2, latex_hline = 'major') %>%
